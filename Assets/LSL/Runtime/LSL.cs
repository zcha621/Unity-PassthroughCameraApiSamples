    ...
    public class StreamInlet : LSLObject
    {
        ...
        */
        ...
        public int pull_chunk(float[,] data_buffer, double[] timestamp_buffer, double timeout = 0.0)
        {
            ...
            var data_buffer_flat = new float[numRows * numCols];
        ...
        public int pull_chunk(string[,] data_buffer, double[] timestamp_buffer, double timeout = 0.0)
        {
            ...
            int numRows = data_buffer.GetLength(0);
            int numCols = data_buffer.GetLength(1);
            var data_buffer_flat = new IntPtr[numRows * numCols];
            uint res = dll.lsl_pull_chunk_str(obj, data_buffer_flat, timestamp_buffer, (uint)data_buffer_flat.Length, (uint)timestamp_buffer.Length, timeout, ref ec);
            LSL.check_error(ec);
            try
            {
                for (int i = 0; i < numRows; i++)
                {
                    for (int j = 0; j < numCols; j++)
                    {
                        data_buffer[i, j] = Marshal.PtrToStringAnsi(data_buffer_flat[i * numCols + j]);
                    }
                }
            }
            catch (Exception e)
